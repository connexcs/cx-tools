...
import { updateLocalEnvKey, removeLocalEnvKey } from './sync.js'
...
export async function envSetAction(key, options) {
	...
	try {
		...
		if (result.success) {
			...
			// Update local cx.env file
			try {
				updateLocalEnvKey(key, value)
				if (!silent) {
					console.log('📄 Updated local cx.env file')
				}
			} catch (err) {
				// Don't fail the whole operation if local file update fails
				if (!silent) {
					console.log(`⚠️  Could not update local cx.env file: ${err.message}`)
				}
			}
		} else {
			handleError(result.error, silent)
