...
export async function pushAction(options) {
	try {
		...
		const updatePromises = allToUpdate.map(file => {
		...
		const appId = process.env.APP_ID
		const createPromises = allToCreate.map(file => {
			const config = SYNC_TYPES[file.syncType]
			
			if (!appId) {
				return Promise.resolve({
					file,
					error: new Error('No APP_ID configured. Use "cx configure:app" first.'),
					type: 'create',
					failed: true
				})
			}
		...
		const allOperations = await Promise.all([...updatePromises, ...createPromises])
		
		// Display results
		let successCount = 0
		for (const operation of allOperations) {
			if (operation.failed) {
				console.error(`❌ Failed to ${operation.type} ${operation.file.filename}: ${operation.error.message || operation.error}`)
			} else if (operation.result.success) {
				const verb = operation.type === 'update' ? 'Updated' : 'Created'
				console.log(`✅ ${verb} ${operation.file.filename}`)
				successCount++
			} else {
				console.error(`❌ Failed to ${operation.type} ${operation.file.filename}: ${operation.result.error}`)
			}
		}
