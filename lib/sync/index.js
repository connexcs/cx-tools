...
import { pullItems, pushItems, getAllSyncFiles, deleteSyncFile } from './files.js'
...
export async function pullAction(options) {
	try {
		...
		if (totalItems === 0 && (!envResults.success || envResults.total === 0)) {
		...
		let viewDiffs = false
		const hasDiffs = allItemsWithDiffs.length > 0 || envResults.diffs.length > 0
		...
		const shouldProceed = await confirm({
			message: `Pull ${totalItems} file(s)${envResults.total > 0 ? ` and ${envResults.total} env var(s)` : ''}?`,
			default: true
		})
...
export async function pushAction(options) {
	try {
		...
		if (totalChanges === 0 && envChanges === 0) {
		...
		// Confirm operation
		const shouldProceed = await confirm({
			message: `Push ${totalChanges} file change(s)${envChanges > 0 ? ` and ${envChanges} env var change(s)` : ''}?`,
			default: true
		})
