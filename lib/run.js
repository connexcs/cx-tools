 ...
 * @param {number} sseOptions.logDelay - Delay in ms to wait for late logs after result
 * @returns {Promise<{success: boolean, data?: any, error?: string, contentType?: string}>}
 */
async function executeScriptForge(id, requestBody, silent = false, fn = null, sseOptions = { enabled: true, logDelay: 2000 }) {
	const endpoint = fn ? `scriptforge/${id}/run/${fn}` : `scriptforge/${id}/run`
	...
	return executeWithSSE(endpoint, requestBody, silent, sseOptions.logDelay)
}
 ...
 * @param {Object} requestBody - JSON body for the request
 * @param {boolean} silent - Whether to suppress progress output
 * @param {number} logDelay - Delay in ms to wait for late logs after result
 * @returns {Promise<{success: boolean, data?: any, error?: string, contentType?: string}>}
 */
async function executeWithSSE(endpoint, requestBody, silent, logDelay) {
	...
	try {
		...
		return await parseSSEStream(response, silent, logDelay, controller)
 ...
 * @returns {Promise<{success: boolean, data?: any, error?: string, contentType?: string}>}
 */
async function parseSSEStream(response, silent, logDelay, controller) {
	return new Promise(async (resolve) => {
		...
		const startDelayedFinish = () => {
			if (delayTimeout) return // Already waiting
			
			if (!silent) {
				console.log('─'.repeat(50))
				console.log('✅ Result received')
				console.log(`⏳ Waiting ${logDelay}ms for late logs...`)
			}
		...
		try {
			...
			while (true) {
				...
				if (done) {
					...
					// If we received a result and timeout is pending, let it complete
					// Otherwise finish immediately
					if (!resultReceived) {
						finishWithResult()
					}
					// If result was received, the timeout will call finishWithResult
					break
				}
...
export async function runAction(id, options) {
	...
	const logDelay = parseInt(options.logDelay, 10) || 2000
	...
	try {
		...
		const sseOptions = { enabled: sseEnabled, logDelay }
