...
async function parseSSEStream(response, silent, controller) {
	return new Promise(async (resolve) => {
		...
		const finishWithResult = () => {
			...
			// If an SSE error event was received, resolve as failure
			if (errorResult) {
		...
		const processEvent = (eventType, eventData) => {
			if (eventType === 'error') {
				// Server-sent error event - parse and display
				try {
					const errorData = JSON.parse(eventData)
					const errorName = errorData.name || 'Error'
					const errorMessage = errorData.message || errorData.details?.errorMessage || eventData
					errorResult = `${errorName}: ${errorMessage}`
					if (!silent) {
						console.error(`❌ [${errorName}] ${errorMessage}`)
					}
				} catch (e) {
					errorResult = eventData
					if (!silent) {
